        ...
        if __isRunning:
            if first_move and start_pick_up: # When an object is detected for the first time           
                ...
                if track: # If it is the tracking stage
                    ...
                    time.sleep(0.02)                    
                    track = False
                if start_pick_up: #If the object has not moved for a period of time, start gripping
                    action_finish = False
                    if not __isRunning: # Stop and exit flag detection
                        continue
                    Board.setBusServoPulse(1, servo1 - 280, 500)  #pawsopen
                    # Calculate the angle at which the gripper needs to be rotated
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Open the claws and put down the object
                    time.sleep(0.8)
...
def run(img):
    ...
    #If a recognized object is detected in a certain area, the area will be detected until there is no recognized object.
